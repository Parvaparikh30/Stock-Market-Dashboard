...
#stock market dashboard to show charts of stocks
...
start,end = get_input()
strategy = st.sidebar.selectbox("Select Strategy",('Home','News',"Machine Learning","Technical Analysis"))
...
def functionalities(comp_name,df,strategy):
    if strategy =='Home':
        ...
        news_options = st.sidebar.selectbox("Select Prefered Source",("Headlines","Twitter","Stocktwits"))
        
        if news_options =='Headlines':
            st.header('Headlines')
                ...
                for tweet in tweets:
                    ...
                    for media in tweet.entities.get("media",[{}]):  
                        if media.get("type",None) == "photo":
                            ...
                            st.image(image_content.content)
                    st.write("__________")
                    
        elif news_options=="Stocktwits":
            st.header("Tweets from Stocktwits")
            st.write("___________________________")
            r = requests.get(
                f"https://api.stocktwits.com/api/2/streams/symbol/{comp_name}.json")
            data = r.json()
            for message in data['messages']:
                ...
                try :   
                    st.image(message['links'][0]['image'])
                except:
                    st.write()
                    
                # if (message['links'][0]['image']!='NULL'):    
                #     st.image(message['links'][0]['image'])
                st.write("___________________________________________________")
        
        elif news_options =='Twitter':
                st.header("Tweets of "+ comp_name)
                st.write("___________________________")
                tweets = tw.Cursor(api.search, q=comp_name, lang="en", tweet_mode='extended').items(20)
        ...
        X = np.array(new_df1.drop(['Prediction'], 1))[:-future_days]
        y = np.array(new_df1['Prediction'])[:-future_days]
        x_train, x_test, y_train, y_test = train_test_split(X, y, test_size = 0.25)
        x_future = new_df1.drop(['Prediction'], 1)[:-future_days]
